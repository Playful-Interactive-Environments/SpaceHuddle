...
export default class DetailResult extends Vue {
  ...
  updateState(result: TaskParticipantState[]): void {
    for (const resultType of this.ResultTypeList) {
      ...
      this.resultTypeCountHate[resultType] = 0;
    ...
    for (const state of result) {
      if (state.parameter.resultTypes) {
        this.resultTypeCombinedCount[state.parameter.resultTypes[0]][
          state.parameter.resultTypes[1]
        ] += 1;
        for (let i = 0; i < this.ResultTypeList.length; i++) {
          this.resultTypeCount[state.parameter.resultTypes[i]][i] += 1;
        }
        for (const item of Object.keys(state.parameter.resultTypeValues)) {
          if (state.parameter.resultTypeValues[item] < 0) {
            this.resultTypeCountHate[item] += 1;
          }
        }
      }
    }
  ...
  get resultData(): any {
    ...
    if (this.showDetails) {
      ...
      if (this.showException) {
        datasets.push({
          label: this.$t(
            `module.information.personalityTest.${this.test}.participant.exception`
          ),
          data: Object.keys(this.resultTypeCount).map(
