...
class SessionController extends Controller
{
    ...
    public function readByKey(string $session_key): object
    {
        $query = "SELECT * FROM session WHERE connection_key = :session_key";
        $stmt = $this->connection->prepare($query);
        $stmt->bindParam(":session_key", $session_key);
        $stmt->execute();
        return (object)$this->database->fetchFirst($stmt, "Wrong Session Key");
    }
    ...
    public function add(
        ...
        $login_id = Authorization::getAuthorizationProperty("login_id"); # check if the user is logged in
        $connection_key = $this->generateNewSessionKey();
        $params = $this->formatParameters(
            [
                "title" => ["default" => $title],
                "connection_key" => ["default" => $connection_key],
                "max_participants" => ["default" => $max_participants],
                "expiration_date" => ["default" => $expiration_date]
            ]
        );
    ...
    public function update(
        ...
        $params = $this->formatParameters(
            [
                ...
                "max_participants" => ["default" => $max_participants],
                "expiration_date" => ["default" => $expiration_date],
                "public_screen_module_id" => ["default" => $public_screen_module_id]
            ]
        );
